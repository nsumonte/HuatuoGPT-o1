...
def train(args):
    ...
    # Crear dataset y dataloader de entrenamiento
    ...
    # Crear dataloader de validación si se proporciona
    val_dataloader = None
    if args.val_data_path:
        val_args = argparse.Namespace(**vars(args))
        val_args.data_path = args.val_data_path
        val_dataset = Train_dataset(val_args, tokenizer)
        ...
        accelerator.print(f'✓ Validation dataset cargado: {len(val_dataset)} ejemplos')
    ...
    def evaluate(eval_dataloader):
        """Evaluar el modelo en el validation set."""
        model.eval()
        eval_metric = SFTMetric(device=torch.cuda.current_device())
        ...
        with torch.no_grad():
            for batch in eval_dataloader:
                ...
                eval_metric(output.logits, labels, loss)
        
        eval_acc, eval_loss = eval_metric.get_metric()
        eval_perplexity = torch.exp(torch.tensor(eval_loss)).item()
        
        model.train()
        return eval_acc, eval_loss, eval_perplexity
    ...
    def save_checkpoint(epoch, step, global_step):
        ...
        if accelerator.is_main_process:
            ...
            # Copiar archivos de configuración del modelo original (solo si es directorio local)
            copy_files = []
            if os.path.isdir(args.model_path):
                ...
                print(f'Modelo guardado en {output_dir} (model_path es HuggingFace Hub, no se copian archivos adicionales)')
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for batch_cnt, batch in train_dataloader_iterator:
            ...
            if global_step % args.log_every == 0 and accelerator.is_main_process:
                ...
                tokens_per_second = num_tokens_in_batch / time_elapsed if time_elapsed > 0 else 0
                
                # Gradientes (antes del optimizer.step)
                total_norm = 0.0
                ...
                seq_length_mean = (input_ids != tokenizer.pad_token_id).sum(dim=1).float().mean().item()
                seq_length_max = (input_ids != tokenizer.pad_token_id).sum(dim=1).max().item()
                ...
                wandb.log({
                    ...
                    'train/optimizer_step_skipped': int(accelerator.optimizer_step_was_skipped),
                    
                    # Progreso
                    'train/epoch': epoch,
                    'train/global_step': global_step,
                    'train/batch_step': batch_cnt,
                    
                    # Throughput
                    'performance/tokens_per_second': tokens_per_second,
                    'performance/total_tokens_processed': total_tokens_processed,
                    'performance/steps_per_second': 1.0 / time_elapsed if time_elapsed > 0 else 0,
                    'performance/time_per_step': time_elapsed,
                    
                    # Memoria GPU
                    'memory/gpu_allocated_gb': memory_allocated,
                    'memory/gpu_reserved_gb': memory_reserved,
                    'memory/gpu_max_allocated_gb': max_memory_allocated,
                    
                    # Batch statistics
                    'batch/size': batch_size_actual,
                    'batch/seq_length_mean': seq_length_mean,
                    'batch/seq_length_max': seq_length_max,
                    'batch/tokens_in_batch': num_tokens_in_batch,
                }, step=global_step)
                
                # Reset timer para el siguiente step
                step_start_time = time.time()
            
            # Evaluación periódica si se especifica
            if args.eval_every > 0 and global_step % args.eval_every == 0 and val_dataloader and accelerator.is_main_process:
                accelerator.print(f'\n🔍 Evaluando en validation set (step {global_step})...')
                eval_acc, eval_loss, eval_perplexity = evaluate(val_dataloader)
                ...
                accelerator.print(f'✓ Eval Loss: {eval_loss:.4f} | Eval Acc: {eval_acc:.4f} | Eval Perplexity: {eval_perplexity:.2f}\n')

        # Evaluación al final de cada epoch
        if val_dataloader and accelerator.is_main_process:
            accelerator.print(f'\n🔍 Evaluando al final del epoch {epoch+1}...')
            eval_acc, eval_loss, eval_perplexity = evaluate(val_dataloader)
            
            wandb.log({
                'eval_epoch/loss': eval_loss,
                'eval_epoch/accuracy': eval_acc,
                'eval_epoch/perplexity': eval_perplexity,
                'eval_epoch/epoch': epoch,
            }, step=global_step)
            
            accelerator.print(f'✓ Epoch {epoch+1} Eval - Loss: {eval_loss:.4f} | Acc: {eval_acc:.4f} | Perplexity: {eval_perplexity:.2f}\n')
    ...
    if accelerator.is_main_process:
        training_time_total = time.time() - step_start_time
        
        # Calcular estadísticas finales
        final_acc, final_loss = metric.get_metric(reset=False)
        final_perplexity = torch.exp(torch.tensor(final_loss)).item()
        avg_tokens_per_second = total_tokens_processed / training_time_total if training_time_total > 0 else 0
        
        # Log de resumen final
        wandb.log({
            'summary/final_loss': final_loss,
            'summary/final_accuracy': final_acc,
            'summary/final_perplexity': final_perplexity,
            'summary/total_steps': global_step,
            'summary/total_epochs': args.n_epochs,
            'summary/total_tokens_processed': total_tokens_processed,
            'summary/avg_tokens_per_second': avg_tokens_per_second,
            'summary/total_training_time_hours': training_time_total / 3600,
        })
        ...
        print(f"✓ Total tokens procesados: {total_tokens_processed:,}")
        print(f"✓ Throughput promedio: {avg_tokens_per_second:.2f} tokens/s")
        print(f"✓ Tiempo total: {training_time_total/3600:.2f} horas")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_data_path', type=str, default=None, help='Ruta al dataset de validación JSON (opcional)')
    parser.add_argument('--max_samples', type=int, default=0, help='Limitar número de muestras (0=sin límite)')
    parser.add_argument('--eval_every', type=int, default=0, help='Evaluar cada N steps (0=solo al final de epoch)')
